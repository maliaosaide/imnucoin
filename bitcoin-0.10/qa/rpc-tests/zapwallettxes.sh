...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars IMNUCOIND and IMNUCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

IMNUCOIND=${IMNUCOIND:-${1}/imnucoind}
CLI=${IMNUCOINCLI:-${1}/imnucoin-cli}
...
$IMNUCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$IMNUCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
